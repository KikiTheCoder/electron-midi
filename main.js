const { app, BrowserWindow, ipcMain, Tray, Menu, nativeImage } = require("electron")
const path = require("path")
const midi = require("midi")
const { Voicemeeter, StripProperties } = require("voicemeeter-connector")

const input = new midi.Input()
const output = new midi.Output();

const MIDI_IN = 0
const MIDI_OUT = 1

var win
var voicemeeter
var tray

const createWindow = () => {
    win = new BrowserWindow({
        width: 800,
        height: 600,
        webPreferences: {
            nodeIntegration: true,
            devTools: true,
            preload: path.join(__dirname, "preload.js")
        },
    })

    var icon = nativeImage.createFromDataURL('data:image/png;base64,')
    tray = new Tray(icon)

    const contextMenu = Menu.buildFromTemplate([
        { label: 'Show', type: 'normal', click: () => win.show() },
        { label: 'Hide', type: 'normal', click: () => win.hide() },
        { label: 'Close', type: 'normal', click: () => exitApp() },
    ])

    tray.setToolTip('Kiki VU Meter')
    tray.setContextMenu(contextMenu)

    tray.on("click", () => {
        win.show()
    })

    win.setAutoHideMenuBar(true)
    win.setMenuBarVisibility(false)
    win.loadFile('index.html')

    win.on("minimize", () => {
        win.hide()
    })

    input.on("message", (delta, msg) => {
        console.log(`m: ${msg}`);
        win.webContents.send("receive-midi", msg)
    })
}

console.log("----- Inputs -----");
for (let i = 0; i < input.getPortCount(); i++) {
    console.log(`Port Nr ${i}: ${input.getPortName(i)}`);
}

console.log("----- Outputs -----");
for (let i = 0; i < output.getPortCount(); i++) {
    console.log(`Port Nr ${i}: ${output.getPortName(i)}`);
}

input.openPort(MIDI_IN);
output.openPort(MIDI_OUT);

(async () => {
    voicemeeter = await Voicemeeter.init()
    voicemeeter.connect()
})()

app.whenReady().then(() => {
    createWindow()

    ipcMain.on("send-midi", (event, msg) => {
        output.sendMessage(msg)
    })

    ipcMain.on("setGain", (event, gain) => {
        voicemeeter.setStripParameter(0, StripProperties.Gain, gain);
    })

    app.on('activate', () => {
        if (BrowserWindow.getAllWindows().length === 0) createWindow()
    })
})

const exitApp = () => {
    input.closePort()
    voicemeeter.disconnect();
    app.quit()
}

app.on('window-all-closed', () => {
    if (process.platform !== 'darwin') {
        exitApp()
    }
})